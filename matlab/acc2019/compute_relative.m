...
if params.bearing_only == true
    ...
    for i = 1:size(target_truth_bo,2)
        ...
        if i > 1 && i < size(target_truth_bo,2)
            ...
            target_truth_bo(5:7,i) = R_i2b*vt_i - aircraft_bo(9:11,i);
        end
    end
    ...
    % Rotate true velocity into body-level frame for bearings-only
    ...
    for i = 1:size(target_truth_rb,2)
        roll = roll_from_q(aircraft_rb(5:8,i));
        pitch = pitch_from_q(aircraft_rb(5:8,i));
        R_l2b = R_v2_to_b(roll)*R_v1_to_v2(pitch);
    ...
    % Rotate true velocity into body-level frame for range+bearings
