...
% Load data
true_state = reshape(fread(fopen(strcat(['/tmp/',name,'_ekf_truth.log']), 'r'), 'double'), 1 + 15, []); % [time;pos;vel;euler;acc_bias;gyro_bias]
ekf_state = reshape(fread(fopen(strcat(['/tmp/',name,'_ekf_est.log']), 'r'), 'double'), 1 + 15, []); % [time;pos;vel;euler;acc_bias;gyro_bias]
ekf_cov = reshape(fread(fopen(strcat(['/tmp/',name,'_ekf_cov.log']), 'r'), 'double'), 1 + 15, []); % [time;pos;vel;att;acc_bias;gyro_bias]
